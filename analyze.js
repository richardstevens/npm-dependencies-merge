...
function analyze (options, callback) {
  ...
  function eachDir (err, dir) {
    ...
    var fileToSearch = options.file || 'package.json'
    var pkgjson = path.join(dir, fileToSearch)
    pending++
      ...
      if (!result[dep]) {
        ...
        if (result[dep].version != version) {
          if (compare(result[dep].version, version) < 0) {
            ...
            log.info('analyze', '%s: inconsistent dependency version %s@%s', dir, dep, version)
          }
        }
